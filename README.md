...
- **性能压测（含加密）**：网页 `/api/benchmark` 可选 明文 / AES-256-CBC / ChaCha20 / 三种全跑。实测每模式连发 100 封 ~1MB 邮件：发送 100/100、丢包率 0%、加密邮件逐封解密还原 100/100；AES 平均单封约 100ms、ChaCha20 约 60ms（远低于 2s 指标），测完自动清理；支持客户端线程池并发发送（`threads=1/2/4/8/16`）与多账号并发发送（`accountCount=1/2/3/4` 选择发件账号数量，线程数固定由 `threads` 决定，账号只作为发件身份轮询使用），并将指标拆分为总用时、SMTP 传输时间、加密时间、解密时间、传输速度（封/s）和平均每封传输用时，可对比单线程与多线程吞吐。
