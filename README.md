...
# 附录：contrib/crypto-project —— 邮件加密子系统（README 原文）

> 以下内容原样保留自 `contrib/crypto-project` 分支的顶层 README.md。
> 该分支为独立根提交（与 main 无共同祖先），提交信息：feat(crypto): 邮件加密子系统完整源码（AES-256-CBC + RSA-2048 数字信封），供 MailForge 集成参考。
> 其源码现已并入本分支仓库根目录：`Makefile`、`common/`、`crypto/`、`tests/`、`tools/`、`docs/`、`practice/`。
