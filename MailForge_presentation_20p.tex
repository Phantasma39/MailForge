    ...
    \begin{column}{0.52\textwidth}
      \begin{block}{MailForge 是什么}
        ...
        \begin{itemize}
          ...
          \item 线程池 + 多账号并发压测；账号密码 PBKDF2-SHA256 随机盐哈希存储。
          \item 现代协议扩展：ESMTP AUTH PLAIN、POP3 CAPA/UIDL/TOP。
        \end{itemize}
      \end{block}
      \begin{block}{运行约定}
        \small
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
        ...
        \item 本项目实现核心命令，并支持 ESMTP AUTH PLAIN；POP3 侧支持 CAPA、UIDL、TOP。STARTTLS/公网投递受环境限制暂未启用。
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{block}{SMTP 对话}
...
\begin{frame}{端到端数据流与测试结果}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{性能压测}
        \small
        每模式 100 封约 1MB；支持 \code{threads=1/2/4/8/16} 与多账号轮询。实测单线程约 15.7 封/s，4 线程约 54.3 封/s，丢包率 0\%。
      \end{block}
      \begin{alertblock}{注意}
        \small
