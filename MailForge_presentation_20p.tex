...
\begin{frame}
  ...
  \vfill
  \begin{center}
    \small 原生 Socket $\cdot$ 手写 SMTP/POP3/HTTP $\cdot$ 自研 AES/ChaCha20 $\cdot$ 两层加密
  \end{center}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.50\textwidth}
      \begin{block}{落盘与多用户隔离}
        ...
        \end{enumerate}
      \end{block}
      \begin{alertblock}{改进点}
        \small
        \code{rand()} 不是线程安全，文件名可能碰撞；应使用 CSPRNG 或原子计数。
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{POP3 实现：快照 + 会话状态}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{alertblock}{改进点}
        \small
        未实现 \code{UIDL/TOP/APOP}；并发删除无保护；认证与传输均为明文。
      \end{alertblock}
    \end{column}
  \end{columns}
...
protected:
  virtual void handleClient(int fd) = 0;
      ...
      \begin{alertblock}{改进点}
        \small
        一连接一线程、\code{detach()}、无优雅退出；建议线程池 / epoll / jthread。
      \end{alertblock}
    \end{column}
    \begin{column}{0.50\textwidth}
      \begin{block}{HTTP 后端}
...
\begin{frame}{层2：自研 AES-256-CBC 与 ChaCha20}
  ...
  \end{alertblock}
  \note{对比分组密码和流密码。主动指出 CBC/ChaCha20 无 MAC 是最大密码学改进点。}
\end{frame}
...
void ChaCha20::Block(key, nonce, counter, out) {
  ...
  for (int i = 0; i < 16; ++i)
    Store32LE(work[i] + state[i], out + 4*i);
      ...
      \begin{alertblock}{改进点}
        \small
        流密码无完整性认证，当前靠解密后是否以 \code{Subject:} 开头兜底；应使用 ChaCha20-Poly1305，且同一密钥下 nonce 绝不能重复。
      \end{alertblock}
    \end{column}
  \end{columns}
...
\end{frame}
% ============================== 19 改进方向 ==============================
\begin{frame}{还可以修改的地方：按优先级}
  \scriptsize
  \renewcommand{\arraystretch}{0.92}
  \begin{table}
    \centering
    \begin{tabular}{p{0.14\textwidth}p{0.44\textwidth}p{0.23\textwidth}p{0.10\textwidth}}
      \toprule
      类别 & 问题 & 建议 & 优先级 \\
      \midrule
  ...
  \vspace{0.2em}
  \begin{center}
    \small 下一步三件事：升级 AEAD、密码哈希与 STARTTLS、线程池与流式加解密。
  \end{center}
  \note{诚实讲不足是加分项。优先讲 P0 安全类，再讲架构和测试。}
\end{frame}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{高频问答}
        ...
        \begin{itemize}
          ...
          \item 中间人风险：GCM tag 防篡改，但信封缺签名/证书，仍需改进。
        \end{itemize}
      \end{block}
      \begin{block}{谢谢}
        \centering
